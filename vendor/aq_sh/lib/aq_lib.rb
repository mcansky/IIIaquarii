module AqLib
  ...
  # Don't lost to expand path...
  config_file = File.expand_path("../../../../config/settings.yml", __FILE__)
  ...
  dbconfig = YAML::load(File.open(File.expand_path("../../../../config/database.yml", __FILE__), "r"))
  ...
  class AqRepository < ActiveRecord::Base
    ...
    def aqlog(message)
      File.open(Settings.aq_sh.user_home + "/" + Settings.aq_sh.user_name + "/" + Settings.log.aq_sh, "a") do |log|
  ...
  class Command
    ...
    def aqlog(message)
      File.open(Settings.aq_sh.user_home + "/" + Settings.aq_sh.user_name + "/" + Settings.logs.aq_sh, "a") do |log|
        log.puts Time.now.strftime("%d/%m/%y %H:%M ") + message
      end
    end
      ...
      if command.check(sh_command)
        ...
        if !repo_path.empty?
          ...
          if a_repo
            ...
            command.aqlog("Couldn't find the repository #{repo_name} (#{repo_path})")
            exit(1)
          end
